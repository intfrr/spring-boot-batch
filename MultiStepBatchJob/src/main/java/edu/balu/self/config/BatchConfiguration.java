...
public class BatchConfiguration {
	...
	public FlatFileItemReader<Player> playerReader(){
       ...
       // lineTokenizer.setNames("ID","lastName","firstName","position","birthYear","debutYear");
		...
		return new FlatFileItemReaderBuilder<Player>()
				...
				.resource(new ClassPathResource("input/player.csv"))
	...
	public FlatFileItemReader<Game> gameReader(){
        DelimitedLineTokenizer lineTokenizer= new DelimitedLineTokenizer();
        lineTokenizer.setNames(new String[] {"id","year","team","week","opponent","completes","attempts","passingYards","passingTd","interceptions","rushes","rushYards","receptions","receptionYards","totalTd"});
       ...
       // lineTokenizer.setNames("ID","lastName","firstName","position","birthYear","debutYear");
		...
		return new FlatFileItemReaderBuilder<Game>()
				...
				.resource(new ClassPathResource("input/games.csv"))
				.lineMapper(lineMapper)
				.lineTokenizer(lineTokenizer)				
				//.delimited()
				//.names(new String[] {"id","year","team","week","opponent","completes","attempts","passingYards","passingTd","interceptions","rushes","rushYards","receptions","receptionYards","totalTd"})
    ...
    public JdbcPlayerSummaryDao summaryWriter(DataSource dataSource){
	   ...
	   @Bean
	    protected Step summarizationStep() {
		   return stepBuilderFactory.get("summarizationStep")
		            .<PlayerSummary, PlayerSummary> chunk(10)
	 ...
	 /*  @Bean
	    protected Step playerSummarization(Tasklet tasklet) {
	        return stepBuilderFactory.get("summarizationStep")
	            .
	            .build();
	    }
	   */
	    ...
	    private static final String QUERY_PLAYER_SUMMARY=
